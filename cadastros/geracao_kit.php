...
try {
  ...
  // }, 100);
      ...
      if (step === 5) {
        const numeracaoKit =
        window.recebe_acao === "editar"
          ? window.recebe_id_kit
          : window.recebe_codigo_kit_gerado;

        $("#numeracao-kit").html(numeracaoKit ?? "");
...
function restaurarModelosSelecionados() {
  ...
  if (window.recebe_acao === 'editar') {
    try {
      ...
      console.warn('⚠️ Erro ao interpretar modelos_documentos:', err);
      modelos = [];
    }
  } else {
    // 🔹 Se não for editar, usa os dados mais recentes das variáveis globais
    if (Array.isArray(window.smModelosSelecionadosCompletos) && window.smModelosSelecionadosCompletos.length > 0) {
      modelos = window.smModelosSelecionadosCompletos.map(m => m.text);
    } else if (Array.isArray(window._modelosSelecionados)) {
      modelos = window._modelosSelecionados.slice();
    } else {
      modelos = [];
    }
  }
  ...
  window._modelosSelecionados = selecionados;
  window.smModelosSelecionadosCompletos = selecionados.map(text => ({ text, checked: true }));

  // 🔹 Atualiza interface
  if (typeof updateSelectedList === 'function') updateSelectedList();

  console.log('✅ Restaurado modelos selecionados:', window._modelosSelecionados);
}
...
debugger;
  ...
  $.ajax({
    ...
    error: function(xhr, status, error) { console.log("Erro:", error); }
            ...
            function inicializarAgenciaContaSelectsKit() {
  try {
    var selAgExames = document.getElementById('agencia-conta-exames-select');
    var selAgTreinamentos = document.getElementById('agencia-conta-treinamentos-select');
    var selAgEpi = document.getElementById('agencia-conta-epi-select');

    function enviarAgenciaContaKitPorSelect(tipo_orcamento, recebe_agencia_conta) {
      debugger;
      var processoKit = 'incluir_valores_kit';
      if (window.recebe_acao === 'editar') {
        processoKit = 'atualizar_kit';
      ...
      if (window.recebe_acao === 'editar' && window.recebe_id_kit) {
        dataKit.valor_id_kit = window.recebe_id_kit;
      }
      ...
      $.ajax({
        ...
        success: function(retorno_exame_geracao_kit) {
          try {
            console.group('AJAX > sucesso inclusão/atualização valor kit via select agência/conta');
            console.log('Retorno:', retorno_exame_geracao_kit);
            console.groupEnd();
          } catch (e) {}
        ...
        error: function(xhr, status, error) {
          console.log('Falha ao incluir/atualizar kit (agência/conta): ' + error);
        },
        complete: function() {
          try {
            console.log('AJAX > inclusão/atualização valor kit via select agência/conta finalizado');
          } catch (e) {}
        }
      });
    }
    ...
    if (selAgExames) {
      selAgExames.addEventListener('change', function() {
        debugger;
        var value = this.value || '';
        if (!value) return;

        var partes = value.split('|');
        var agencia = (partes[0] || '').trim();
        var conta = (partes[1] || '').trim();
        if (!agencia || !conta) return;

        var recebe_agencia_conta = 'Ag ' + agencia + ' • C/C ' + conta;

        // guarda AGENCIA/CONTA global para EXAMES/PROCEDIMENTOS
        window.dado_bancario_agencia_conta_exames_procedimentos = recebe_agencia_conta;

        enviarAgenciaContaKitPorSelect('exames_procedimentos', recebe_agencia_conta);
      });
    }

    if (selAgTreinamentos) {
      selAgTreinamentos.addEventListener('change', function() {
        var value = this.value || '';
        if (!value) return;
        ...
        enviarAgenciaContaKitPorSelect('treinamentos', recebe_agencia_conta);
      });
    }

    if (selAgEpi) {
      selAgEpi.addEventListener('change', function() {
        var value = this.value || '';
        if (!value) return;
        ...
        enviarAgenciaContaKitPorSelect('epi_epc', recebe_agencia_conta);
      });
    }
  } catch (e) {
    console.error('Erro ao inicializar selects de agência/conta do kit:', e);
  }
}
    ...
    function initializeRiscosComponent() {
      ...
      if (quickRiskBtnSalvar && quickRiskModal && selectedRisksContainer && !quickRiskBtnSalvar._bound) {
        ...
        quickRiskBtnSalvar.addEventListener('click', function() {
          ...
          // Garante que a variável global risksData (mesma de buscar_riscos) exista
          if (typeof risksData !== 'object' || !risksData) {
            risksData = {};
          }

          // Mantém window.risksData sincronizado com a variável global
          if (typeof window.risksData !== 'object' || !window.risksData) {
            window.risksData = risksData;
          }
          ...
          // Se o grupo ainda não existir, cria em ambos (compartilhando o mesmo objeto)
          if (!risksData[grupo]) {
            const grupoObj = {
              name: nomeGrupo,
              risks: []
            };
            risksData[grupo] = grupoObj;
            // mantém referência igual também no window.risksData
            window.risksData[grupo] = grupoObj;
          }
          ...
          console.log('Risco rápido adicionado a risksData (global):', {
            grupo,
...
// 🔹 PRIORIDADE 1: Sessão atual (usuário marcou riscos manualmente)
if (typeof window.selectedRisks === 'object' && Object.keys(window.selectedRisks).length > 0) {
  riscosFonte = Object.entries(window.selectedRisks).map(([codigo, info]) => ({
    codigo,
    descricao: info.name,
    grupo: info.group
...
// 🔹 PRIORIDADE 2: Snapshot (usuário trocou de aba)
else if (Array.isArray(window._snapshotRiscosSelecionados) && window._snapshotRiscosSelecionados.length > 0) {
  riscosFonte = window._snapshotRiscosSelecionados.map(r => ({
    codigo: r.codigo || '',
    descricao: r.descricao || '',
...
// 🔹 PRIORIDADE 3: Edição (dados vindos do banco)
else if (window.recebe_acao === "editar" && window.kit_riscos) {
  let kitRiscosArray = window.kit_riscos;
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      etapas = [
        ...
        <div class="fat-section">
          <h1 class="fat-header">KIT:<span id="numeracao-kit"></span></h1>
...
if ((window.aptAptidoesSelecionadas && window.aptAptidoesSelecionadas.length > 0) ||
  ...
  // 🔹 Seleciona array de base preferindo sempre os 'itens' do UI
  let baseArray;
  if (Array.isArray(itens)) {
    baseArray = itens;
  } else if (window.recebe_acao === "editar") {
    baseArray = tipo === "aptidao"
  ...
  if (tipo === "exame") {
    const renderArray = Array.isArray(exames_selecionados) ? exames_selecionados : [];
    const total = renderArray.reduce((soma, item) => {
      let v = item?.valor ?? "0";
    ...
    console.log("💰 Total de exames:", window.fatTotalExames);
  }

  // 🔹 Renderiza badges na interface
  const renderArray = tipo === 'aptidao' ? aptidoes_selecionadas : exames_selecionados;
...
async function atualizarSelecionados(checkbox, tipo) {
  ...
  const codigo = String(checkbox.value);
  ...
  const arraySelecionado =
    tipo === 'aptidao'
      ? (Array.isArray(aptAptidoesSelecionadas)
          ...
          : (Array.isArray(window.aptAptidoesSelecionadas)
              ? (aptAptidoesSelecionadas = window.aptAptidoesSelecionadas)
              : (aptAptidoesSelecionadas = [])))
      : (Array.isArray(aptExamesSelecionados)
          ? aptExamesSelecionados
          : (Array.isArray(window.aptExamesSelecionados)
              ? (aptExamesSelecionados = window.aptExamesSelecionados)
              : (aptExamesSelecionados = [])));
  ...
  if (estadoInicialMarcado) {
    ...
    if (!arraySelecionado.some(a => String(a.codigo) === String(codigo))) {
      arraySelecionado.push(item);
    }

    if (!recebe_valores_exames_selecionados.some(ex => String(ex.codigo) === String(codigo))) {
      recebe_valores_exames_selecionados.push({ codigo: String(codigo), valor });
    }
  } else {
    // Remove se existir
    const index = arraySelecionado.findIndex(a => String(a.codigo) === String(codigo));
    if (index !== -1) arraySelecionado.splice(index, 1);

    const idxValor = recebe_valores_exames_selecionados.findIndex(a => String(a.codigo) === String(codigo));
...
function renderizarCheckboxes() {
  ...
  if (checkboxContainerApt) {
    ...
    aptDadosAptidoes.forEach(item => {
      ...
      if (aptSelecionadas.some(a => String(a.codigo) === String(item.codigo))) {
        emAtualizacaoProgramatica = true;
        input.checked = true;
        emAtualizacaoProgramatica = false;
      }
  ...
  if (checkboxContainerExames) {
    ...
    aptDadosExames.forEach(item => {
      ...
      const input = checkbox.querySelector('input[type="checkbox"]');
      if (examesSelecionados.some(e => String(e.codigo) === String(item.codigo))) {
        emAtualizacaoProgramatica = true;
        input.checked = true;
        emAtualizacaoProgramatica = false;
      }
