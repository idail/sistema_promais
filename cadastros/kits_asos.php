...
<div>
    <h1 class="dashboard">KITS ASOs</h1>
</div>
...
<script>
    ...
    // 🔹 Função auxiliar para verificar se o kit contém algum modelo permitido
    function contemModeloPermitido(kit) {
        try {
            let modelos = [];
            if (typeof kit.modelos_selecionados === "string") {
                modelos = JSON.parse(kit.modelos_selecionados);
            } else if (Array.isArray(kit.modelos_selecionados)) {
                modelos = kit.modelos_selecionados;
            }

            return modelos.some(m => modelosPermitidos.includes(m));
        } catch (e) {
            console.warn("Erro ao processar modelos_selecionados:", e);
            return false;
        }
    }

    // 🔹 Filtra apenas kits com status "rascunho" E modelos permitidos
    const kitsFiltrados = kits.filter(kit => {
        const statusEhRascunho =
            kit.status && kit.status.toLowerCase() === "rascunho";
        return statusEhRascunho && contemModeloPermitido(kit);
    });

    if (kitsFiltrados.length > 0) {
        for (let index = 0; index < kitsFiltrados.length; index++) {
            let kit = kitsFiltrados[index];
            ...
            row.innerHTML = `
                ...
                <td style="text-align:center; vertical-align:middle;">Rascunho</td>
