...
<div>
    <h1 class="dashboard">Treinamento/Capacitação</h1>
</div>
...
<div>
    <table id="treinamento_capacitacao_tabela">
        <thead>
            <tr>
                <th>ID</th>
                <th>Código</th>
...
<script>
    ...
    let recebe_tabela_treinamento_capacitacao;
    $(document).ready(function() {
        // Função para buscar dados da API
        function buscar_treinamento_capacitacao() {
            $.ajax({
                url: "cadastros/processa_treinamento_capacitacao.php", // Endpoint da API
                method: "GET",
                dataType: "json",
                data: {
                ...
                success: function(resposta_treinamento_capacitacao) {
                    ...
                    if (resposta_treinamento_capacitacao.length > 0) {
                        ...
                        preencher_tabela(resposta_treinamento_capacitacao);
        ...
        function preencher_tabela(treinamento_capacitacao) {
            debugger;
            let tbody = document.querySelector("#treinamento_capacitacao_tabela tbody");
            tbody.innerHTML = ""; // Limpa o conteúdo existente

            if (treinamento_capacitacao.length > 0) {
                for (let index = 0; index < treinamento_capacitacao.length; index++) {
                    let valor_treinamento_capacitacao = treinamento_capacitacao[index];

                    let row = document.createElement("tr");
                    row.innerHTML = `
                        <td>${valor_treinamento_capacitacao.id}</td>
                        <td>${valor_treinamento_capacitacao.codigo_treinamento_capacitacao}</td>
                        <td>${valor_treinamento_capacitacao.nome}</td>
                ...
                $("#treinamento_capacitacao_tabela tbody").append("<tr><td colspan='9' style='text-align:center;'>Nenhum registro localizado</td></tr>");
            }
        }

        // Função para inicializar o DataTables
        function inicializarDataTable() {
            recebe_tabela_treinamento_capacitacao = $("#treinamento_capacitacao_tabela").DataTable({
                "language": {
        ...
        buscar_treinamento_capacitacao();

        $(".search-bar").on('keyup', function() {
            recebe_tabela_treinamento_capacitacao.search(this.value).draw();
        });
